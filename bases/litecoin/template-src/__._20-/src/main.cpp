...
uint256 hashGenesisBlock("0x__._50-");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(__._0-); // __._3-: starting difficulty is 1 / 2^12
...
int64 CTransaction::nMinTxFee = __._1-;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = __._2-;
...
const string strMessageMagic = "__._3- Signed Message:\n";
...
{
    // __._3-: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = __._4-;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / __._5-); // __._3-: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = __._6-; // __._3-: 3.5 days
static const int64 nTargetSpacing = __._7-; // __._3-: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // __._3-: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // __._3-: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // __._3-: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x__._8-");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "__._9-";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = __._4-;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("__._10-") << OP_CHECKSIG;
        CBlock block;
        ...
        assert(block.hashMerkleRoot == uint256("0x__._16-"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // __._3-: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("__._3-Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("__._3-Miner : ProcessBlock, block not accepted");
    }
